...
----------------------------------------------------------------------
CREATE TABLE movies (
    movie_id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
...
CREATE TABLE cinemas (
    cinema_id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
...
CREATE TABLE rooms (
    room_id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
...
CREATE TABLE seats (
    seat_id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
...
CREATE TABLE showtimes (
    showtime_id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
...
CREATE TABLE booking_details (
    booking_detail_id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
...
CREATE TABLE food_combos (
    food_id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
    name NVARCHAR(100) NOT NULL,
    description NVARCHAR(MAX),
...
CREATE TABLE booking_foods (
    booking_food_id UNIQUEIDENTIFIER PRIMARY KEY DEFAULT NEWSEQUENTIALID(),
    booking_id UNIQUEIDENTIFIER REFERENCES bookings(booking_id) ON DELETE CASCADE,
    food_id UNIQUEIDENTIFIER REFERENCES food_combos(food_id),
    quantity INT NOT NULL DEFAULT 1,
    price DECIMAL(10, 2) NOT NULL -- Lưu lại giá tại thời điểm mua
);
GO
...
CREATE TABLE cinematches (
    ...
    showtime_id UNIQUEIDENTIFIER REFERENCES showtimes(showtime_id),
    seat_id UNIQUEIDENTIFIER REFERENCES seats(seat_id),
    adjacent_seat_id UNIQUEIDENTIFIER REFERENCES seats(seat_id),
    booking_id UNIQUEIDENTIFIER REFERENCES bookings(booking_id),
    matched_booking_id UNIQUEIDENTIFIER REFERENCES bookings(booking_id),
...
-- Thêm Indexes cho các Foreign Keys để tối ưu JOIN
...
CREATE INDEX idx_cinematches_showtime ON cinematches(showtime_id);
CREATE INDEX idx_cinematches_user ON cinematches(user_id);
CREATE INDEX idx_cinematches_booking ON cinematches(booking_id);
GO
